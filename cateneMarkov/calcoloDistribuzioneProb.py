...
def ricercaStatiUscita(stato, md):
            ...
            if n != 0:
                ...
                for i,val in enumerate(col):
                    ...
                    # CASO IN CUI SI FINISCE IN UNA STAZIONE ERLANG
                        ...
                        """Funzione per andare alla ricerca dell'oggetto Stato che contiene lo stato=stazOut"""
                        mapStatoVal=[ricercaOggettoStato(md,tuple(stazOut),"erlang",md.stazioni[val].k),valQ]
    ...
    # Lo stato preso in considerazione risulta essere "Erlang"
        ...
        for j,n in enumerate(stato.stato):
            ...
            col = []
            if n!=0:
                # PARTENZA da una stazione erlang
                if (md.stazioni[j].tipo=="erlang"):

                    # Lo stato da cui si parte ha S>1 e quindi tocca scalare..
                    if stato.listStazErl[0]['stadK']>1:
                        print "Sono nello stato:",stato.listStazErl,"basta scalare"
                        valQ=(md.stazioni[stato.listStazErl[0]['indStaz']].k)/(md.stazioni[j].s)
                    ...
                    # Lo stato da cui si parte ha S==1 e quindi tocca cercare nuovi stati
                    elif stato.listStazErl[0]['stadK']==1:
                        ...
                        for i,val in enumerate(col):
                            ...
                            valQ=((md.stazioni[stato.listStazErl[0]['indStaz']].k)/(md.stazioni[j].s))*md.q[j][val]

                            # CASO IN CUI SI N==1 (era l'ultima persona presente nella stazione Erlang)
                            if n==1:
                                print "Era l'ultima persona presente nella stazione Erlang! Esco dalla CPU!"
                                """Funzione per andare alla ricerca dell'oggetto Stato che contiene lo stato=stazOut"""
                                mapStatoVal=[ricercaOggettoStato(md,tuple(stazOut),"normale"),valQ]
                                print "Lo stato out e:",md.spazioStati[mapStatoVal[0]].stato
                            ...
                            # CASO IN CUI SI N>1 (non era l'ultima persona)
                                ...
                                """Funzione per andare alla ricerca dell'oggetto Stato che contiene lo stato=stazOut"""
                                mapStatoVal=[ricercaOggettoStato(md,tuple(stazOut),"erlang",md.stazioni[j].k),valQ]
                                print "Lo stato out e:",md.spazioStati[mapStatoVal[0]].stato,",listStazErl:",md.spazioStati[mapStatoVal[0]].listStazErl
                        ...
                        if md.stazioni[j].k>1:
                            # Calcolo valore di uscita dallo stato preso in considerazione da inserire in Q
                            valQ=((md.stazioni[stato.listStazErl[0]['indStaz']].k)/(md.stazioni[j].s))*md.q[j][j]
                            listaStat.append([ricercaOggettoStato(md,tuple(stato.stato),"erlang",md.stazioni[j].k),valQ])
                ...
                # PARTENZA da una stazione non Erlang
                    ...
                    for val in col:
                        ...
                        print "Si rimane in una stazione Erlang"
                        """Funzione per andare alla ricerca dell'oggetto Stato che contiene lo stato=stazOut"""
...
def ricercaOggettoStato(md,stazOut,tipo,k=0):
    for i,stato in md.spazioStati.items():
        # La chiamata e stata fatta per la ricerca di uno stato Erlang e lo stato preso in considerazione e di Erlang
        if (tipo=="erlang")and(stato.tipo=="erlang"):
